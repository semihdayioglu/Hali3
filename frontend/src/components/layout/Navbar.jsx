...
// Navbar linkleri
const navLinks = [
  { id: 'haliflex', name: 'HALİFLEKS', slug: null, hasDropdown: false, isActive: true },
  { id: 'cim-hali', name: 'ÇİM HALI', slug: 'cim-hali', hasDropdown: true },
  { id: 'karo-hali', name: 'KARO HALI', slug: 'karo-hali', hasDropdown: true },
  { id: 'pvc-zemin', name: 'PVC ZEMİN', slug: 'pvc-zemin', hasDropdown: true },
  { id: 'paspas', name: 'PASPAS', slug: 'paspas', hasDropdown: true },
  { id: 'aksesuar', name: 'AKSESUAR', slug: 'aksesuar', hasDropdown: true },
];
...
const Navbar = () => {
  ...
  const isActive = (slug) => {
    return slug && location.pathname.includes(`/kategori/${slug}`);
  };
  ...
  return (
    ...
    >
          ...
          <div className="flex items-center gap-8">
            {navLinks.map((link) => (
              <div key={link.id} className="relative">
                {link.slug ? (
                  <Link
                    to={`/kategori/${link.slug}`}
                    data-testid={`nav-link-${link.id}`}
                    onMouseEnter={() => link.hasDropdown && handleMouseEnter(link.id)}
                    className={`py-4 font-medium text-sm uppercase tracking-wide transition-colors flex items-center gap-1
                      ${isActive(link.slug) 
                        ? 'text-[#4A7C4E] border-b-2 border-orange-500' 
                        : 'text-[#2C3E2D] hover:text-[#4A7C4E]'
                      }`}
                  >
                    {link.name}
                  ...
                  <button
                    data-testid={`nav-link-${link.id}`}
                    className={`py-4 font-semibold text-sm uppercase tracking-wide transition-colors
                      ${link.isActive 
                        ? 'text-[#2C3E2D] border-b-2 border-orange-500' 
                        : 'text-[#2C3E2D] hover:text-[#4A7C4E]'
                      }`}
                  >
                    {link.name}
      ...
      <div className="md:hidden">
        ...
        {mobileMenuOpen && (
          <div data-testid="mobile-nav-menu" className="border-t border-gray-100 bg-white">
            {navLinks.filter(link => link.slug).map((link) => (
              <div key={link.id} className="border-b border-gray-50">
                <button
                  onClick={() => setActiveDropdown(activeDropdown === link.id ? null : link.id)}
                  className={`w-full flex items-center justify-between px-4 py-3 text-left uppercase text-sm font-medium
                            ${isActive(link.slug) ? 'bg-[#4A7C4E] text-white' : 'text-[#2C3E2D]'}`}
                >
                  <span>{link.name}</span>
                  {link.hasDropdown && (
                ...
                {activeDropdown === link.id && dropdownData[link.id] && (
                  <div className="bg-[#F5F8F2] px-4 py-4">
                    <div className="grid grid-cols-3 gap-3">
                      {dropdownData[link.id].items.slice(0, 6).map((item, index) => (
                        <Link key={index} to={`/urun/${item.slug}`} className="space-y-2 group">
                          <div className="aspect-square rounded-lg overflow-hidden bg-white">
                            <img src={item.img} alt={item.name} className="w-full h-full object-cover" loading="lazy" />
                          </div>
                          <p className="text-xs text-[#2C3E2D] text-center font-medium line-clamp-1">{item.name}</p>
                        </Link>
                      ))}
                    </div>
                    <Link
                      to={`/kategori/${link.slug}`}
                      className="mt-4 block w-full py-2.5 bg-[#4A7C4E] text-white text-center rounded-full text-sm font-medium"
                    >
                      Tümünü Gör
                    </Link>
                  </div>
